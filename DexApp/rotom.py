...
def search(search):
    ...
    if len(dex_search) > 0:
        ...
        if search.lower() in dex_search:
            return {"dex": [names[search.lower(), search.title()]]}, True, len(dex_search)
        count += len(dex_search)
    ...
    if len(move_search) > 0:
        ...
        if search.lower() in [ m.lower() for m in move_search ]:
            ...
            return {"moves": themove}, True, len(move_search)
        count += len(move_search)
    ...
    if len(ability_search) > 0:
        ...
        if search.lower() in [ a.lower() for a in ability_search ]:
            ...
            return {"ability": theability}, True, len(ability_search)
        count += len(ability_search)
